...
int main(int argc, char** argv)
{
	...
	nh.param<double>("transX", transX, 0.635);	// transX is distance (robot coordinate) from marker to center of robot
	nh.param<double>("transY", transY, 0.0);	// transY is lateral distance from center of marker to center of robot
	nh.param<double>("transZ", transZ, 0.157);	// transZ is height of marder respect to ground
	...
	while(nh.ok()){
		...
		try{
			ar_tool_listener.lookupTransform("camera", "4x4_1",  
        						ros::Time(0), cTb);
			rTc = rTb * cTb.inverse();
			if(i == 10){
